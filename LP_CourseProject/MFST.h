...
#define MFST_TRACE7		*log.stream<<std::setw(4)<<std::left<<state[k].lenta_position<<": "\
							<<std::setw(20)<<std::left<<rule.getCRule(rbuf,state[k].nrulechain)\
							<<std::endl;
#define MFST_TRACE_START	std::cout	<< std::setw( 4) << std::left << "Кроу" << ':'\
										<< std::setw(20) << std::left << " Правіла"\
										<< std::setw(30) << std::left << " Уваходныя стужка"\
										<< std::setw(20) << std::left << " Стэк" << std::endl;

typedef std::deque<short> MFSTSTSTACK;
namespace MFST
{
	struct MfstState
	{
	...
	struct Mfst
	{
		...
		LT::LexTable lex;
		MFSTSTSTACK st;
		std::deque<MfstState> storestate;
		Mfst();
