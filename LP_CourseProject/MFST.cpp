...
namespace MFST
{
	...
	bool Mfst::start()
	{
		...
		case LENTA_END:
			...
			sprintf_s(buf, MFST_DIAGN_MAXSIZE, "Cінтаксічны аналіз выкананы без памылак");
	...
	char* Mfst::getCSt(char* buf)
	{
		...
		{
			short p = st[k];
			buffer[st.size() - 1 - k] = GRB::Rule::Chain::alphabet_to_char(p);
		}
		buffer[st.size()] = 0x00;
		buf = buffer;
		return buf;
	...
	{
		MfstState* state = new MfstState[storestate.size()];
		GRB::Rule rule;

		for (unsigned short k = 0; k < storestate.size(); ++k)
		{
			state[k] = storestate[k];
			rule = grebach.getRule(state[k].nrule);
			char rbuf[256];
			MFST_TRACE7
		}
	}
